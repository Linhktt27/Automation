...
public class LoginSmo {
    ...
    public void i_fill_username_invalid() {
        login.fillUsername("gfghff");
    }
    @When("I fill password")
    public void i_fill_password() {
        login.fillPassword(comd.passMain);
    }
    ...
    @Then("I login fail")
    public void i_login_fail() {
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        String mgs = wait.until(ExpectedConditions.visibilityOfElementLocated(login.TxtError)).getText();
        Assert.assertEquals(mgs,"Tài khoản của Quý khách chưa được kích hoạt");
    }

    @After
    public void afterTest (Scenario scenario){
        try {
            base.tearDown(scenario,"login.feature");
        } catch (Exception e) {
